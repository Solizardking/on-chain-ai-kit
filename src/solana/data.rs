...
pub async fn fetch_pair_info(mint_or_symbol: String) -> Result<PairInfo> {
    ...
    matching_pairs
        ...
        .ok_or_else(|| anyhow::anyhow!("No matching pairs found for {}", mint_or_symbol))
}
...
pub async fn holdings_to_portfolio(holdings: Vec<Holding>) -> Result<Vec<PortfolioItem>> {
    ...
    let metadata_futures: Vec<_> = holdings
        ...
        .map(|holding| {
            let client = &client;
            async move {
                let url = format!("https://tokens.jup.ag/token/{}", holding.mint);
    ...
    let prices_url = format!("https://api.jup.ag/price/v2?ids={}", mints.join(","));
    let price_response: PriceResponse = client.get(&prices_url).send().await?.json().await?;
    ...
    let portfolio: Vec<PortfolioItem> = holdings
        ...
        .map(|(holding, metadata)| {
            let price = price_response
                ...
                .get(&holding.mint)
                .map(|p| match p {
                    Some(price_data) => price_data.price.parse::<f64>().unwrap_or(0.0),
                    None => 0.0,
                })
                .unwrap_or(0.0);

            let amount = holding.amount as f64 / (10f64.powi(metadata.decimals as i32));
...
mod tests {
    ...
    async fn test_holdings_to_portfolio() {
        let holdings = get_holdings(&make_rpc_client(), &load_keypair_for_tests().pubkey())
            .await
            .unwrap();
