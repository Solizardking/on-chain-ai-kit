...
pub trait TransactionSigner: Send + Sync {
    ...
    async fn sign_and_send_encoded_solana_transaction(&self, _tx: String) -> Result<String> {
        Err(anyhow::anyhow!(
            "Solana transactions not supported by this signer"
        ))
    }
    ...
    #[cfg(feature = "evm")]
    async fn sign_and_send_json_evm_transaction(&self, _tx: serde_json::Value) -> Result<String> {
        Err(anyhow::anyhow!(
            "EVM transactions not supported by this signer"
        ))
    }
