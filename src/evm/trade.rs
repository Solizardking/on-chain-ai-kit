...
pub async fn create_trade_tx(
    ...
    // Parse input amount
    let amount_in =
        CurrencyAmount::from_raw_amount(input_token.clone(), BigInt::from_str(&input_amount)?)
    ...
    let trade = Trade::from_route(route.clone(), amount_in, TradeType::ExactInput)
...
mod tests {
    use super::*;
    use crate::evm::util::{execute_evm_transaction, make_provider, with_local_evm_signer};
    ...
    async fn test_approval() {
        ...
        let input_token = "0x82aF49447D8a07e3bd95BD0d56f35241523fBab1".to_string();

        with_local_evm_signer(execute_evm_transaction(move |owner| async move {
            create_approve_tx(
                input_token,
    ...
    async fn test_trade_evm() {
        ...
        //  WETH on arbitrum
        let output_token = "0x82aF49447D8a07e3bd95BD0d56f35241523fBab1".to_string();
        // USDC on arbitrum
        let input_token = "0xaf88d065e77c8cC2239327C5EDb3A432268e5831".to_string();
        // 1 usdc
        let input_amount = "1000000".to_string();

        with_local_evm_signer(execute_evm_transaction(move |owner| async move {
            create_trade_tx(input_token, input_amount, output_token, &provider, owner).await
        }))
        .await
        .unwrap();
    }
