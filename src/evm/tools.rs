...
")]
pub async fn verify_swap_router_has_allowance(token_address: String) -> Result<bool> {
...
")]
pub async fn approve_token_for_router_spend(input_token_address: String) -> Result<String> {
    let provider = make_provider()?;
...
")]
pub async fn transfer_eth(recipient: String, amount: String) -> Result<String> {
    execute_evm_transaction(move |owner| async move {
        create_transfer_eth_tx(recipient, amount, &make_provider()?, owner).await
    })
    .await
}
...
pub async fn transfer_erc20(
    ...
    execute_evm_transaction(move |owner| async move {
        create_transfer_erc20_tx(token_address, recipient, amount, &make_provider()?, owner).await
    })
    .await
}
...
pub async fn get_eth_balance(address: String) -> Result<String> {
    wrap_unsafe(move || async move { balance(&make_provider()?, address).await }).await
}

#[tool]
pub async fn get_erc20_balance(token_address: String, address: String) -> Result<String> {
    wrap_unsafe(
        move || async move { token_balance(address, token_address, &make_provider()?).await },
    )
    .await
}
